...
const petalImages = [
  { src: '/modeling/CloseUp.JPEG', altText: 'Close up photo' },
  { src: '/modeling/FullBody.JPEG', altText: 'Full body photo' },
  { src: '/modeling/SideZoomOut.JPEG', altText: 'Side zoom out photo' },
  { src: '/modeling/Side.JPEG', altText: 'Side profile photo' },
  { src: '/modeling/SideBody.JPEG', altText: 'Side body photo' },
  { src: '/modeling/TorsoThighB.JPEG', altText: 'Torso and thigh B photo' },
];
...
export default function ModelingPage() {
  ...
  const centerpieceScale = 1.2;
  const petalScale = 1.15;
  const petalOrbitRadius = 55;
  const petalWidth = "20%";
  const centerpieceWidth = "40%";
  ...
  if (!isAuthenticated) {
    return (
      <main className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-warm-white p-8 rounded-lg shadow-lg border border-pink-accent/30">
          <h1 className="font-serif text-2xl text-deep-brown text-center mb-6">
            Protected Content
          </h1>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-pink-accent/50 rounded-md focus:outline-none focus:ring-2 focus:ring-pink-accent bg-white text-deep-brown"
            ...
            <button
              type="submit"
              className="w-full bg-pink-accent text-white py-2 px-4 rounded-md hover:bg-pink-accent/80 transition-colors"
            >
              Enter
            </button>
          </form>
          <Link href="/" className="block text-center mt-4 text-pink-accent hover:underline text-sm">
            Back to Home
          </Link>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-white text-black flex flex-col">
      ...
      <div className="container mx-auto p-4 flex-grow">
        ...
        <p className="text-center text-sm text-gray-500 mb-8">Modeling Portfolio</p>
